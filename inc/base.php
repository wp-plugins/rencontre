...
function profil_plus($a2,$a3,$a4,$a5)
	...
	if ($a3=="c_categ")
		...
		for($v=0;$v<count($c4);++$v)
			...
			if($v==0)
				{
				$wpdb->query("INSERT INTO ".$wpdb->prefix."rencontre_profil (c_categ,c_label,t_valeur,i_type,i_poids,c_lang) VALUES('".$c4[$v]['b']."','*** A MODIFIER ***','',1,0,'".$c4[$v]['a']."')");
				$lastid = $wpdb->insert_id;
				}
			else $wpdb->query("INSERT INTO ".$wpdb->prefix."rencontre_profil (id,c_categ,c_label,t_valeur,i_type,i_poids,c_lang) VALUES('".$lastid."','".$c4[$v]['b']."','*** A MODIFIER ***','',1,0,'".$c4[$v]['a']."')");
			}
		}
	else if ($a3=="c_label") 
		{
